...
export default function App() {
    ...
    async function loadSession() {
      ...
      try {
        ...
        if (data?.authenticated) {
          setAuthReady(true);
          setAuthUserEmail(String(data.email || "").trim());
        } else {
          setAuthReady(false);
          setAuthUserEmail("");
        }
  ...
  async function requestOtp() {
    const email = normalizedAuthEmail();
    if (!email) {
      setAuthError("Enter your Ciklum email.");
      return;
    }
    if (!email.endsWith("@ciklum.com")) {
      setAuthError("Only @ciklum.com emails are allowed.");
      return;
    }
    ...
    try {
      ...
      setAuthChallengeToken(String(data?.challengeToken || ""));
      setAuthCodeSent(true);
      setAuthInfo(`Code sent to ${email}.`);
    } catch (err) {
  ...
  async function verifyOtp() {
    const email = normalizedAuthEmail();
    const code = String(authOtp || "").trim();
    if (!email || !code || !authChallengeToken) {
    ...
    try {
      const res = await fetch("/api/auth/verify-otp", {
        ...
        body: JSON.stringify({
          email,
          code,
          challengeToken: authChallengeToken,
        }),
      });
      const data = await res.json();
      ...
      setAuthReady(true);
      setAuthUserEmail(String(data?.email || email).trim());
      setAuthOtp("");
      setAuthChallengeToken("");
  ...
  async function logout() {
    try {
      ...
      // Ignore logout transport issues; clear local state regardless.
    }
    setAuthReady(false);
    setAuthUserEmail("");
    setAuthOtp("");
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ fontSize: 12, color: "var(--ck-muted)", marginBottom: 16 }}>
            Sign in with your Ciklum email to continue. Access is restricted to <strong>@ciklum.com</strong>.
          </div>

          <label style={{ display: "block", fontSize: 11, color: "var(--ck-muted)", marginBottom: 6 }}>Work email</label>
          <input
            type="email"
            value={authEmail}
            onChange={(e) => setAuthEmail(e.target.value)}
            placeholder="name@ciklum.com"
            autoComplete="email"
            style={{
              width: "100%",
          ...
          {authCodeSent && (
            <>
              <label style={{ display: "block", fontSize: 11, color: "var(--ck-muted)", marginBottom: 6 }}>One-time code</label>
              <input
                type="text"
                value={authOtp}
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {!authCodeSent ? (
              ...
              >
                {authSubmitting ? <><Spinner size={10} color="#fff" /> Sending...</> : "Send OTP"}
              </button>
            ) : (
              <>
                <button
                ...
                >
                  {authSubmitting ? <><Spinner size={10} color="#fff" /> Verifying...</> : "Verify OTP"}
                </button>
                <button
                  type="button"
                  onClick={() => { void requestOtp(); }}
      ...
      <div style={{
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8, flexWrap: "wrap" }}>
          <div style={{
            ...
            <span style={{ fontSize: 11, color: "var(--ck-muted)" }}>{authUserEmail || "signed-in user"}</span>
            <button
              type="button"
              onClick={() => { void logout(); }}
              style={{
